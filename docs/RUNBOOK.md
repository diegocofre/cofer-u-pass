...
> Headless execution is adapter-gated. In v1.0.2, the official ChatGPT, Gemini, and DeepSeek adapters remain visible-only until supervised provider smoke tests demonstrate reliable headless authentication and execution. `profiles status --verify` follows the adapter's authentication-verification policy rather than forcing headless mode.
